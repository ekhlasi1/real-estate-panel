...
async function renderAdvisors() {
    ...
    if (adminGrid) {
        if (advisors.length === 0) {
            ...
            adminGrid.innerHTML = advisors.map((a, idx) => {
                const cardNum = a.cardNumber || (idx + 1);
                const cardUrl = `${location.origin}/${cardNum}`;
                return `
                <div class="advisor-card">
                    <div class="advisor-avatar">${a.image ? `<img src="${safeAttr(a.image)}" />` : `<i class="fas fa-user-tie"></i>`}</div>
                    <h4>${escapeHtml(a.name)}</h4>
                    ...
                    <div style="margin-top:8px;display:flex;gap:6px;align-items:center;justify-content:center;flex-wrap:wrap;">
                        <a href="${cardUrl}" target="_blank" style="font-size:11px;color:var(--green-primary);text-decoration:underline;">
                            <i class="fas fa-id-card"></i> کارت ویزیت: /${cardNum}
                        </a>
                        <button type="button" onclick="navigator.clipboard.writeText('${cardUrl}');this.textContent='کپی شد ✓';setTimeout(()=>{this.innerHTML='<i class=\\'fas fa-copy\\'></i>';},1500);" style="font-size:11px;border:1px solid var(--gray-300);background:none;border-radius:6px;padding:2px 6px;cursor:pointer;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (advisorForm) {
        advisorForm.addEventListener('submit', async function(e) {
            ...
            const englishName = document.getElementById('advisorEnglishName')?.value.trim() || '';
            const cardNumberRaw = document.getElementById('advisorCardNumber')?.value.trim() || '';
            const cardNumber = cardNumberRaw ? parseInt(cardNumberRaw, 10) : null;
        ...
        changeForm.addEventListener('submit', async function(e) {
            ...
            try {
                const res = await fetch('/api/change-password', {
                    ...
                    body: JSON.stringify({
                        role: userType,
                        advisorId: userType === 'advisor' ? advisorId : undefined,
                ...
                if (result.success) {
                    error.textContent = '✅ رمز عبور با موفقیت تغییر کرد';
                    error.style.color = '#28a745';
                    error.style.display = 'block';
                    setTimeout(() => { window.location.href = '/login.html'; }, 2000);
                } else {
                    error.textContent = '❌ ' + (result.error || 'خطا در تغییر رمز عبور');
                    error.style.color = '';
                    error.style.display = 'block';
                    setTimeout(() => { error.style.display = 'none'; }, 3000);
                }
        ...
        loginForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    localStorage.setItem('realestate_user_type', result.role);
                    if (result.role === 'advisor') {
                        localStorage.setItem('realestate_advisor_id', result.advisorId);
                        localStorage.setItem('realestate_advisor_name', result.advisorName);
                    } else {
                        localStorage.removeItem('realestate_advisor_id');
                        localStorage.removeItem('realestate_advisor_name');
                    }
                    ...
                    error.textContent = '❌ ' + (result.error || 'نام کاربری یا رمز عبور اشتباه است');
                    error.style.display = 'block';
                    setTimeout(() => { error.style.display = 'none'; }, 3000);
                }
            } catch (err) {
