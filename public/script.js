...
function persianToEnglishNumber(str) {
    ...
    return str.replace(persianNumbers, function(match) {
        return englishNumbers[match];
    });
}
...
function toPersianNumber(num) {
    ...
    return String(num).replace(/\d/g, function(match) {
...
// ============================================
// FORMAT NUMBER INPUT (پشتیبانی از فارسی و انگلیسی)
// ============================================
function formatNumberInput(e) {
    ...
    // حذف همه کاراکترهای غیرعددی (فقط اعداد)
    value = value.replace(/[^0-9]/g, '');
    if (value) {
        const number = Number(value);
        // نمایش با کاما به فارسی
        const formatted = formatNumberWithCommas(number);
        input.value = toPersianNumber(formatted);
    } else {
        input.value = '';
    }
}
...
async function renderProperties(filter = 'all') {
    ...
    grid.innerHTML = await Promise.all(visible.map(async (p) => {
        ...
        let priceDisplay = '';
        if (p.type === 'فروش') {
            priceDisplay = `قیمت: ${formatPrice(p.price)} تومان`;
        } else {
            const rentPrice = p.rentPrice || p.price || 0;
            const monthlyPrice = p.monthlyPrice || 0;
            priceDisplay = `رهن: ${formatPrice(rentPrice)} تومان | اجاره: ${formatPrice(monthlyPrice)} تومان`;
        }
    ...
    if (grid) {
        ...
        grid.innerHTML = displayList.map(a => {
            const imageHtml = a.image
                ? `<img src="${a.image}" class="advisor-image" alt="${a.name}" onerror="this.className='advisor-image-placeholder';this.outerHTML='<div class=\\'advisor-image-placeholder\\'><i class=\\'fas fa-user-tie\\'></i></div>'" />`
                : `<div class="advisor-image-placeholder"><i class="fas fa-user-tie"></i></div>`;
...
async function renderAdminProperties() {
    ...
    tbody.innerHTML = props.map(p => {
        ...
        if (p.type === 'فروش') {
            ...
            const rentPrice = p.rentPrice || p.price || 0;
            priceDisplay = `رهن ${formatPrice(rentPrice)} / اجاره ${formatPrice(p.monthlyPrice || 0)}`;
        }
    ...
    container.innerHTML = contacts.map(c => `
        <div class="contact-item">
            <div class="info">
                ...
                ${c.advisorName ? `<span style="color:var(--gold);font-size:12px;">مشاور: ${c.advisorName}</span>` : ''}
...
// ============================================
// COMMISSION CALCULATOR - اصلاح شده با پشتیبانی از اعداد فارسی
// ============================================
function calculateCommission() {
    ...
    // استفاده از تابع کمکی برای تبدیل اعداد فارسی به انگلیسی
    const rentValue = persianToEnglishNumber(rentInput.value.replace(/,/g, '').trim());
    const monthlyValue = persianToEnglishNumber(monthlyInput.value.replace(/,/g, '').trim());
    ...
    // اعتبارسنجی
    if (isNaN(rent)) {
        alert('لطفاً مبلغ رهن را به‌درستی وارد کنید (مثال: ۵۰,۰۰۰,۰۰۰ یا 50000000)');
        rentInput.focus();
        return;
    }
    if (isNaN(monthly)) {
        alert('لطفاً مبلغ اجاره ماهانه را به‌درستی وارد کنید (مثال: ۲,۵۰۰,۰۰۰ یا 2500000)');
        monthlyInput.focus();
        return;
    }

    if (rent < 0) {
        alert('مبلغ رهن نمی‌تواند منفی باشد');
        rentInput.focus();
        return;
    }
    if (monthly < 0) {
        alert('مبلغ اجاره نمی‌تواند منفی باشد');
        monthlyInput.focus();
        return;
    }

    // ۱. کمیسیون از مبلغ رهن
    ...
    // ۲. کمیسیون از مبلغ اجاره
    const commissionFromMonthly = (monthly * 2) / 3;

    // ۳. جمع کل (بدون مالیات)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (propForm) {
        propForm.addEventListener('submit', async function(e) {
            ...
            const imageUrlInput = document.getElementById('propImageUrl').value.trim();
            ...
            if (type === 'فروش') {
                const salePrice = persianToEnglishNumber(document.getElementById('propSalePrice').value.replace(/,/g, '').trim());
                price = parseFloat(salePrice) || 0;
                if (price <= 0) {
                    alert('لطفاً قیمت فروش را به‌درستی وارد کنید');
                    return;
                }
            } else {
                const rent = persianToEnglishNumber(document.getElementById('propRentPrice').value.replace(/,/g, '').trim());
                const monthly = persianToEnglishNumber(document.getElementById('propMonthlyPrice').value.replace(/,/g, '').trim());
                rentPrice = parseFloat(rent) || 0;
                monthlyPrice = parseFloat(monthly) || 0;
                if (rentPrice <= 0 && monthlyPrice <= 0) {
                    alert('لطفاً حداقل یکی از مبالغ رهن یا اجاره را وارد کنید');
                    return;
                }
                price = rentPrice;
            }
            ...
            if (!title || !advisorId) {
                alert('لطفاً عنوان و مشاور را وارد کنید');
                return;
            }
            ...
            if (imageUrlInput) {
                ...
                statusDiv.textContent = '✅ لینک عکس اضافه شد';
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    statusDiv.textContent = '❌ خطا در ثبت ملک: ' + (errorData.error || 'خطای ناشناخته');
                    statusDiv.style.color = '#e74c3c';
                }
            } catch (error) {
    ...
    if (rentInput) {
        rentInput.addEventListener('input', formatNumberInput);
    ...
    if (monthlyInput) {
        monthlyInput.addEventListener('input', formatNumberInput);
        monthlyInput.addEventListener('keyup', function(e) {
            if (e.key === 'Enter') calculateCommission();
        });
    }

    // ---- PRICE INPUT FORMATTING (برای فیلدهای قیمت فروش و اجاره) ----
    const salePriceInput = document.getElementById('propSalePrice');
    const rentPriceInput = document.getElementById('propRentPrice');
    const monthlyPriceInput = document.getElementById('propMonthlyPrice');

    [salePriceInput, rentPriceInput, monthlyPriceInput].forEach(input => {
        if (input) {
