...
// ============================================
// NUMBER INPUT - رفع مشکل پاک شدن اعداد
// ============================================
function formatNumberInput(e) {
    const input = e.target;
    let value = persianToEnglishNumber(input.value);
...
function formatNumberOnBlur(e) {
    ...
    value = value.replace(/[^0-9]/g, '');
    if (value) {
        input.value = toPersianNumber(formatNumberWithCommas(Number(value)));
    } else {
        input.value = '';
    }
}
...
async function updateStats() {
    ...
    document.getElementById('statRented').textContent = props.filter(p => p.status === 'اجاره رفته').length;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (propForm) {
        propForm.addEventListener('submit', async function(e) {
            ...
            if (type === 'فروش') {
                price = parseFloat(persianToEnglishNumber(document.getElementById('propSalePrice').value.replace(/,/g, '').trim())) || 0;
                if (price <= 0) { alert('لطفاً قیمت فروش را وارد کنید'); return; }
            } else {
                rentPrice = parseFloat(persianToEnglishNumber(document.getElementById('propRentPrice').value.replace(/,/g, '').trim())) || 0;
                monthlyPrice = parseFloat(persianToEnglishNumber(document.getElementById('propMonthlyPrice').value.replace(/,/g, '').trim())) || 0;
