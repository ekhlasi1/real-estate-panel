// ============================================
// DATA STORE - با ذخیره‌سازی در سرور (KV)
// ============================================
const API_BASE = '';
...
async function setProperties(props) {
    // این تابع دیگر استفاده نمی‌شود - همه در سرور ذخیره می‌شود
}
...
async function setAdvisors(advs) {
...
async function renderBackupList() {
    // این بخش در نسخه سرور با auto backup کار می‌کند
    const container = document.getElementById('backupList');
    if (!container) return;

    // Backup‌ها در سرور ذخیره نمی‌شوند، فقط نمایش پیام
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Property form (admin) - با آپلود تصویر به سرور
    ...
    if (propForm) {
        propForm.addEventListener('submit', async function(e) {
            ...
            if (response.ok) {
                ...
                statusDiv.textContent = '❌ خطا در ذخیره ملک';
