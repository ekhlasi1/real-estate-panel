...
// WITHOUT R2 - All data stored in KV
// ============================================
...
export default {
    async fetch(request, env) {
        ...
        if (path === '/admin.html') {
            return serveFile(env, 'admin.html');
        }
        if (path === '/style.css') {
            return serveFile(env, 'style.css', 'text/css');
        }
        if (path === '/script.js') {
            return serveFile(env, 'script.js', 'application/javascript');
        }
...
async function serveFile(env, filename, contentType = 'text/html') {
    const file = await env.KV.get(filename);
    if (!file) {
        return new Response('File not found', { status: 404 });
    }
